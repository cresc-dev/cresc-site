...
- You didn't correctly [configure bundleUrl](/docs/getting-started#configuring-the-bundle-url) during setup.
...
#### How does Cresc compare to CodePush and Expo Updates (EAS Update)?

CodePush was officially shut down on March 31, 2025. While many developers looked at Expo Updates (EAS Update) as a potential successor, EAS Update charges on metered usage ($0.005 per additional update MAU and $0.10/GiB edge bandwidth), which often leads to volatile, escalating monthly invoices.

Cresc provides the direct replacement for CodePush and a far more cost-effective alternative to Expo Updates:
- **Price Advantage**: Fixed monthly tiers ($19/mo Standard, $99/mo Pro covering 1,000,000 checks/day) with included global CDN bandwidth and zero per-user overage charges—saving 70% to 90% compared to Expo.
- **Delta Patches (~42 KB)**: Intelligent HDiff differential delivery shrinks update payloads by over 90% vs full bundles.
- **Crash & Rollback Safety**: Automatic native rollback on boot crash plus free source-mapped JS crash reporting.

See our [CodePush & Expo Updates alternative guide](/codepush-alternative) and [Expo pricing vs Cresc breakdown](/expo-pricing-vs-cresc).
...
If Update sizes exceed MB limit metrics, the upload will fail. Upgrade your tier, or practice strict asset [Optimization](bestpractice#optimizing-native-and-hot-update-package-size).
