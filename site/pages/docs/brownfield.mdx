...
The host can also configure and trigger OTA updates without a JS bridge call. Follow [Native Configuration and Updates](/docs/native-api) for `PushyNativeUpdate.configure(...)`, `checkAndUpdate(...)`, explicit Cresc endpoints, and `nativeConfigSource: "native"`. Configure before normal RN startup, or check after an already-completed launch resolution; do not call `getBundleUrl` again merely to initialize checking. Rebuild both the React Native AAR and the host APK/AAB with v10.57.0 or later. The JavaScript example below retains the default JS-owned configuration; use the native guide's ownership option when the host should remain in control.
:::
