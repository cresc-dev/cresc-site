...
interface CrescOptions {
  ...
  // Whether to disable JS error reporting, default: false (enabled)
  // See the "JS Error Monitoring" chapter: https://cresc.dev/docs/errors
  // Requires v10.55.0+
  disableErrorReporting?: boolean;

  // Whether to disable the native cold-start check (the background check
  // native code runs independently after every cold start), default: false
  // (enabled). Disabling it gives up the force-boot brick-rescue channel.
  // See the "Native cold-start check" section below
  // Requires v10.52.1+
...
Since v10.55.0 the SDK reports JavaScript exceptions raised inside an OTA version to the update service, and the console restores the stack to your original source with the source map archived at publish time. Uncaught exceptions are reported automatically (the handler is appended on top of React Native's existing global `ErrorUtils` handler, so existing Sentry-style integrations are untouched), and you can report manually from a `catch`:

```js
crescClient.captureException(error, {
  extra: { screen: "checkout" },
});
```

It is part of every plan at no extra cost. To opt out, set `disableErrorReporting: true` at initialization. The full story — console UI, source map requirements and manual reporting — is in [JS Error Monitoring](errors).

#### Native configuration and manual checks

Since **v10.57.0**, Android, iOS, and HarmonyOS hosts can call native `configure` before JavaScript starts, then `checkAndUpdate` after the normal launch bundle resolution. These APIs prepare updates without displaying dialogs or immediately reloading React Native. Rebuild and distribute the native application when upgrading; a JS-only OTA update cannot add native methods.

See [Native Configuration and Updates](/docs/native-api) for platform examples, result handling, and `nativeConfigSource` ownership rules. **Cresc native configurations must explicitly provide Cresc endpoints**; omitting `endpoints` selects the shared SDK's Pushy preset. The guide includes both Cresc service addresses and the matching discovery URL.

#### Native cold-start check

Since v10.52.1, the client runs an independent background update check from **native code** a few seconds after every cold start, with no JS involved. Its purpose is brick rescue: if a hot update kills your JS, the device can never run the JS-side check again — this native channel is the only one left that can pull the fix back down (activated via the console's [force boot](publish#force-boot-brick-rescue) flag).

The behavior below assumes the default JavaScript-owned configuration (`nativeConfigSource: "javascript"`). With `nativeConfigSource: "native"`, native `configure` controls checking through `disabled` and ordinary activation through `afterDownload`; JS strategy changes do not update these native settings.

Key behaviors:

- **No immediate reload**: native checking downloads and caches updates. With JavaScript-owned settings, silent strategies allow next-launch activation, while `checkStrategy: null` leaves ordinary activation to JS. Server force-boot and existing crash-rescue rules still apply
- **JS reuses its result**: the JS-side check reuses the response the native check just fetched, saving one network round-trip on cold start
- **Safety guards**: the device-local rollback guard has top priority, and first-launch crash protection still rolls back automatically if the activated version fails to start

With JavaScript-owned configuration, disable native checking at init when required by your networking or consent policy:
...
With native-owned configuration, instead pass `disabled: true` with your complete Cresc native configuration. This disables both automatic and manual native checks; it does not disable JavaScript checks. Gate the initial native configuration/startup flow as needed rather than relying only on a JS hook for consent.

Disabling native checking removes this force-boot rescue route. A broken JS update may then require a native app upgrade or reinstall to recover.
...
Downloads an APK and starts installation through Android's `PackageInstaller`. `url` must point directly to the APK file. This feature supports Android 5.0 (API 21) and later.

Installation must be initiated by the user and confirmed in Android's system installer. Regular apps cannot use this API for silent installation. The promise resolves after the system confirmation UI opens; it does not mean the user completed the installation.

Declare the install permission manually in the final app's `AndroidManifest.xml`:

```xml
<uses-permission android:name="android.permission.REQUEST_INSTALL_PACKAGES" />
```

The APK is downloaded into app-private storage and written directly to a `PackageInstaller.Session`, so no external storage permission is required.

On Android 8.0 (API 26) and later, the user must also allow the current app to install unknown apps. If this access has not been granted, the first call opens the app-specific system settings page and rejects with `APK_INSTALL_PERMISSION_REQUIRED`. Call the method again after the user grants access. Once the install session is submitted, the SDK handles `STATUS_PENDING_USER_ACTION` and opens Android's installation confirmation UI. The APK is not installed if the user declines.

:::warning
`REQUEST_INSTALL_PACKAGES` is a sensitive permission. Do not include it in channel builds that do not need in-app APK installation. Google Play does not allow this permission to be used for self-updates; remove it from Google Play builds and direct users to the store or use Google Play's in-app update mechanism instead. Removing this permission only disables full APK installation and does not affect Cresc OTA updates.
:::
...
Reports a JavaScript exception manually. Requires v10.55.0+. `context` accepts `fatal` (whether the error is fatal, default false), `componentStack` (a React component stack) and `extra` (custom context: strings, numbers, booleans and null only, up to 32 fields).

The call returns synchronously and never throws: transport happens in the background, failures are silent, and the same error object is only reported once. Errors are only reported while running an OTA version, and never in development (`__DEV__`). See [JS Error Monitoring](errors).

```js
try {
  await submitOrder(order);
} catch (e) {
  crescClient.captureException(e, { extra: { orderId: order.id } });
}
```
