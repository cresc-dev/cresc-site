...
If you rebuild native code later, you must **change the version number**, and `uploadApk` again. Otherwise, [build timestamp mismatches](/docs/faq#why-do-mismatched-build-timestamps-affect-update-performance) do not block updates but can reduce diff reuse and increase download size.
