...
Since `react-native-update` 10.51.0, the client runs an independent background check from **native code** a few seconds after every cold start, with no JS involved. Force boot is the directive for that channel: even with JS dead, the native check can still pull down the fix and activate it, so the device recovers on its next launch.

### Usage

1. Log in to the [Cresc console](https://cresc.dev)
2. Find the fixed version and publish it with "Full + force boot (rescue)", or for an already-bound version, choose "Force boot (rescue)" from the package menu
3. Review the semantics in the confirmation dialog and publish
...
- **Client requirement**: the native package must ship `react-native-update` >= 10.51.0 (older clients have no native cold-start check, and the console hides the option for them)
- **Native check only**: the interactive JS update flow ignores this flag
